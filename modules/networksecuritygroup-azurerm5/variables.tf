...
DESCRIPTION
  ...
  validation {
    condition = alltrue(
      [
        for _, v in var.diagnostic_settings :
        length(v.log_categories) + length(v.log_groups) + length(v.metric_categories) > 0
      ]
    )
    error_message = "At least one of `log_categories`, `log_groups`, or `metric_categories` must be set for each diagnostic setting."
  }
}
